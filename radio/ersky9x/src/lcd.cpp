...
// for editing : enable the folowing 4 defines so VS-Code editor can gray out 
...
void putsTimexxl( uint16_t x, uint16_t y, int16_t tme, uint8_t att, uint16_t colour )
{
	div_t qr ;
	uint32_t i ;
	// Can't handle negative (yet)
	if ( tme < 0 )
	{
		tme = -tme ;
		lcdDrawCharxxlTransparent( x-10, y, ':' + 1, att, colour ) ;
	}
	lcdDrawCharxxlTransparent( x+84, y, ':', att, colour ) ;

	qr = div( tme, 60 ) ;
	i = qr.quot / 10 ;
	lcdDrawCharxxlTransparent( x+24, y, i + '0', att, colour ) ;
	i = qr.quot % 10 ;
	lcdDrawCharxxlTransparent( x+60, y, i + '0', att, colour ) ;
	i = qr.rem / 10 ;
	lcdDrawCharxxlTransparent( x+112, y, i + '0', att, colour ) ;
	i = qr.rem % 10 ;
	lcdDrawCharxxlTransparent( x+148, y, i + '0', att, colour ) ;
}
#endif
...
#if defined(PCBX12D) || defined(PCBX10)
	lcd_2_digits( x, y, (uint16_t)qr.rem, att2 + LEADING0, colour, background ) ;
#else
...
void lcd_picture( uint16_t i_x, uint16_t i_y, uint16_t maxHeight, uint16_t maxWidth )
{
//	uint16_t *p ;
//	uint32_t rows ;
	uint16_t *src = SdramImage ;
	uint16_t height ;
	uint16_t reduceWidth = 0 ;
...
//		i_y += t ;
	...
	{
		reduceWidth = Image_width - maxWidth ;
	}

	DMAcopyImage( i_x, i_y, Image_width, height, src, reduceWidth ) ;
...
uint16_t lcd_putcAttColour(uint16_t x,uint16_t y,const char c,uint8_t mode, uint16_t colour, uint16_t background )
{
	...
	if ( mode & DBLSIZE )
	{
		return lcd_putcAttDblColour( x, y, c, mode, colour, background ) ;
	}
...
void lcd_putsnAttColour( uint16_t x, uint16_t y, const char * s,uint8_t len,uint8_t mode, uint16_t colour, uint16_t bgColour )
{
	register char c ;
  while(len!=0) {
    c = *s++ ;
    ...
    x = lcd_putcAttColour( x, y, c, mode, colour, bgColour ) ;
    len--;
  }
}
...
#if defined(PCBX12D) || defined(PCBX10)
void lcd_putsAttIdxColour(uint8_t x,uint8_t y,const char * s,uint8_t idx,uint8_t att, uint16_t colour, uint16_t background )
{
	uint8_t length ;
	length = *s++ ;
...
#if defined(PCBX12D) || defined(PCBX10)
uint8_t lcd_putsAttColour( uint8_t x, uint8_t y, const char *s, uint8_t mode, uint16_t colour, uint16_t background )
{
  while(1)
	{
    char c = *s++ ;
...
{
#if defined(PCBX12D) || defined(PCBX10)
  lcd_putsAtt( DisplayOffset, y, s, 0);
#else
  lcd_putsAtt( 0, y, s, 0);
#endif
}
...
{
	lcd_outdezNAtt( x, y, value, attr, 2, colour, background ) ;
}
#else
...
void lcd_hlineStip( uint16_t x, uint16_t y, int16_t w, uint8_t pat )
{
  if(w<0) {x+=w; w=-w;}
...
void lcd_hline( uint16_t x, uint16_t y, int16_t w )
{
...
#if defined(PCBX12D) || defined(PCBX10)
void lcd_rectColour(uint8_t x, uint8_t y, uint8_t w, uint8_t h, uint16_t colour )
{
...
{
	lcdDrawSolidFilledRectDMA( x, y, 1, h, colour) ;
	lcdDrawSolidFilledRectDMA( (x+w-1), y, 1, h, colour) ;
	lcdDrawSolidFilledRectDMA( (x+1), (y+h-1), (w-2), 1, colour) ;
	lcdDrawSolidFilledRectDMA( (x+1), y, (w-2), 1, colour) ;
}

void lcdHiresHline( uint16_t x, uint16_t y, uint16_t w, uint16_t colour )
{
	lcdDrawSolidFilledRectDMA( (x), (y), (w), 1, colour) ;
}
...
void lcdHiresHbar( uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint8_t percent, uint16_t colour )
{
	uint8_t solid ;
	if ( percent > 100 )
	{
	...
	if ( solid )
	{
		lcdDrawSolidFilledRectDMA( x+1, y+1, solid, h, colour) ;
	}
}
...
*/
	...
	const static uint8_t Lcd_init[] =
	{
	  0xe2, //Initialize the internal functions -- not really needed when using reset line
		0xae, //DOF
		0x90, // no fixed lines
		0xc0 |0x4, 	// mirror , 4=X, 8=y
		0x40 |0x0,	// scan line start 0 -- but needs page offset of 4 for this device
		0x81,0x4f,  //contrast setting, a good starting point -- gets overwritten later
		0xAF,  //DON = 1: display ON
		} ;
#endif
...
#ifndef LCD_MAX_X		// NO LCD type specified 
...
// read the inputs, and lock the LCD lines
	...
	while ( TC0->TC_CHANNEL[0].TC_CV < HwDelayScale*HW_COUNT_PER_US*1500 )	// (2500?) 1500 uS, Value depends on MCK/8
	{
		wdt_reset() ;		// Wait
	}
...
extern uint16_t ResetReason ;
	if ( ( ( ResetReason & RSTC_SR_RSTTYP ) != (2 << 8) ) && !unexpectedShutdown )	// Not watchdog
	{
		...
		for ( uint32_t j = 0 ; j < 100 ; j += 1 )
		{
			...
			{
 			  wdt_reset() ;				// Wait, but feed the dog
			}
		}
	}
...
{ 
	uint8_t col_offset ;
	uint16_t delayCount = HwDelayScale*HW_COUNT_PER_US/4 ;		// Value depends on MCK/8
	for ( uint8_t page_addr=0;page_addr <8; page_addr++)
	{

#ifdef LCD_DBG  // for debuging x-y mirror and centring issues
	col_offset = COL_ADDR_OFFSET;
#else 
	col_offset = 0;
#endif
...
#ifdef LCD_DBG
			if ( page_addr < 4)  // some pattern
				pioptr->PIO_ODSR = 0xff;
			else
			{
				if (col_addr >64)
					pioptr->PIO_ODSR = 0xf0;
				else
					pioptr->PIO_ODSR = 0x55	;// set all to 0 -- clear all the LCD memory	
			}
#else
...
// read the inputs, and lock the LCD lines
  ...
  lcdSendCtl(c2);
#endif
	uint8_t c3 = 0xA6 ;		// Display normal (not inverse)
	if (g_eeGeneral.reverseScreen)
	{
		c3 = 0xA7 ;		// Display inverse
	}	
  lcdSendCtl(c3);
#ifndef REVA
#ifdef ARUNI
  ...
  for( lcd_page_addr=0; lcd_page_addr < 8; lcd_page_addr++) // set the next page address and start column in the LCD
	{
		...
		// offsets for ERC12864-2 where 4 for normal and 0 for flipped display
		// these numbers below are for ERC12864-14 display modules

#ifdef ERC12864_14	
		if (g_eeGeneral.rotateScreen)  // offset lines 
    	column_start_lo = COL_ADDR_OFFSET+1;
		else
			column_start_lo = COL_ADDR_OFFSET+3; 	
#elif defined ERC24064_1
			column_start_lo = COL_ADDR_OFFSET;
#else
	if (g_eeGeneral.rotateScreen)  // offset lines --  4 columns for normal ST7565
    	column_start_lo = COL_ADDR_OFFSET;
		else
...
/*
		if (g_eeGeneral.optrexDisplay)		// Don't know what this is for -- Do not see this in the display config screen as an option
...
#else		
			pioptr->PIO_ODSR = lookup[*p++]; 	// copy via this funny lookuptable 
#endif	
